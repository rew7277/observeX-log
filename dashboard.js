...
let _allRows=[],_visibleRows=[],_lastResult=null,_sessions=[],_environments={{ environments|tojson }};
...
function countTo(el,value,suffix=''){
  if(!el)return; const n=Number(String(value).replace(/[^0-9.-]/g,''));
  if(!Number.isFinite(n)){el.textContent=value;return;}
  const start=Number(el.dataset.last||0), t0=performance.now(), dur=420;
  function step(t){const p=Math.min(1,(t-t0)/dur), v=Math.round(start+(n-start)*(1-Math.pow(1-p,3)));el.textContent=v.toLocaleString()+suffix;if(p<1)requestAnimationFrame(step);else el.dataset.last=n;}
...
function renderVisualDashboard(d){
  ...
  let warn=total?Math.max(0,Math.min(100,(warns/total)*100)):0;
  let ok=Math.max(0,100-err-warn);
  let score=Math.max(0,Math.min(100,Math.round(d.health_score||100)));
  ...
  countTo(document.getElementById('donut-score'),score);
  ...
  let mttr=errors>50||err>10?'High':errors>0||err>2?'Medium':'Low';
...
function applyClientSearch(){
  ...
  const needle=[ql.trace, ql.text.join(' ')].filter(Boolean).join(' ').toLowerCase();
  ...
  renderRows(rows);
  const info=byId('logs-page-info'); if(info) info.textContent=`${rows.length} matching log(s)`;
}
...
function quickSearch(q){const el=byId('filter-q'); if(el) el.value=q||''; applyClientSearch(); showSection('logs');}
