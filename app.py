        ...
        if not lat:
            ...
            if len(times) >= 2:
                try:
                    ...
                    lat = ""
        # MuleSoft HTTP listener format example:
        # [s-paymentengine-api].get:\paymentEngine\loanDetails:s-paymentengine-api-config.CPU_LITE
        mule_req = re.search(r"\[([A-Za-z0-9_.-]+)\]\.([A-Za-z]+):\\?([^:\]]+)", line)
        method = ""
        endpoint = ""
        if mule_req:
            if not app or app == "unknown":
                app = mule_req.group(1)
                current_app = app
            method = mule_req.group(2).upper()
            endpoint = "/" + mule_req.group(3).replace("\\", "/").strip("/ ")
            endpoint = re.sub(r"/{2,}", "/", endpoint)

        flow = extract_first([r"processor:\s*([^;\]]+)", r'"FlowName"\s*:\s*"([^"]+)"', r"\]\.([a-zA-Z0-9_-]+flow)\."], line, "")
        processor_index = extract_first([r"/processors/(\d+)", r"processors/(\d+)"], flow or line, "")
        rows.append({
            ...
            "event": trace, "flow": flow, "method": method, "endpoint": endpoint,
            "processor_index": processor_index, "status": status, "latency": int(lat) if str(lat).isdigit() else 0,
...
# ── VNext System Map override: MuleSoft-aware architecture extraction ─────────
def _parse_dt_ms(value: str):
    try:
        if not value:
            return None
        v = str(value).replace(',', '.')
        if len(v) == 12 and v.count(':') == 2:  # HH:MM:SS.mmm
            t = datetime.datetime.strptime(v, "%H:%M:%S.%f")
            return ((t.hour*3600 + t.minute*60 + t.second) * 1000) + int(t.microsecond/1000)
        return int(datetime.datetime.fromisoformat(v.replace('Z','')).timestamp() * 1000)
    except Exception:
        return None

def _arch_clean_flow_name(value: str) -> str:
    """Keep real Mule/application names; remove only processor indexes and log noise."""
    s = str(value or '').strip().strip('"\'[]{}(),;')
    if not s:
        return ''
    s = re.sub(r'\\[tnr]', ' ', s)
    ...
    s = re.sub(r'\.CPU_[A-Z_]+.*$', '', s, flags=re.I)
    s = re.sub(r':.*$', '', s) if s.lower().startswith(('before ', 'after ', 'info ', 'debug ', 'warn ', 'error ')) else s
    s = re.sub(r'\b(?:before|after)\s+[a-z ]+\s+log\b.*$', '', s, flags=re.I)
    s = re.sub(r'[^A-Za-z0-9_.:-]+', '-', s)
    s = re.sub(r'-{2,}', '-', s).strip('-_.:')
    return s[:80]

def _arch_endpoint_from_row(row: dict) -> str:
    ep = (row.get('endpoint') or '').strip()
    if ep:
        return _normalise_endpoint(ep)
    msg = row.get('message') or ''
    # MuleSoft listener: [api].get:\paymentEngine\loanDetails:config
    m = re.search(r'\[[A-Za-z0-9_.-]+\]\.([A-Za-z]+):\\?([^:\]]+)', msg)
    if m:
        return _normalise_endpoint('/' + m.group(2).replace('\\', '/').strip('/ '))
    m = re.search(r'\b(?:GET|POST|PUT|DELETE|PATCH|OPTIONS|HEAD)\s+(/[A-Za-z0-9/_\-.{}?=&%]+)', msg, re.I)
    if m:
        return _normalise_endpoint(m.group(1))
    return '/'

def _arch_method_from_row(row: dict) -> str:
    if row.get('method'):
        return str(row.get('method')).upper()
    msg = row.get('message') or ''
    m = re.search(r'\[[A-Za-z0-9_.-]+\]\.([A-Za-z]+):\\?', msg)
    if m:
        return m.group(1).upper()
    m = re.search(r'\b(GET|POST|PUT|DELETE|PATCH|OPTIONS|HEAD)\b', msg, re.I)
    return m.group(1).upper() if m else ''

def _arch_flow_from_row(row: dict) -> str:
    flow = row.get('flow') or ''
    flow = _arch_clean_flow_name(flow)
    if flow:
        return flow
    msg = row.get('message') or ''
    m = re.search(r'processor:\s*([A-Za-z0-9_.:/-]+)', msg, re.I)
    return _arch_clean_flow_name(m.group(1)) if m else ''

def _arch_action_from_msg(msg: str) -> str:
    low = (msg or '').lower()
    if 'before loan details log' in low or re.search(r'\bbefore\b', low):
        return 'request-start'
    if 'after loan details log' in low or re.search(r'\bafter\b', low):
        return 'response-success' if '"message":"success"' in low or 'success' in low else 'response'
    return 'log'

def extract_architecture_graph(rows: list, raw: str, env: str, session_id: int, user_id: int, api_name: str = '', endpoint: str = '') -> dict:
    """
    MuleSoft-aware topology:
    - treats [app].method:\endpoint as API + endpoint
    - treats processor subflow as Service
    - keeps 'before/after ... log' as trace actions, NOT architecture nodes
    - computes latency by trace/event first-to-last timestamp when explicit ms latency is missing
    """
    node_map, edge_map, traces = {}, {}, {}

    def add_node(name, tier, row=None):
        name = _arch_clean_flow_name(name)
        if not name:
            return ''
        ...
        if row:
            ...
            n["errors"] += 1 if lvl in ("ERROR", "FAILURE") else 0
            n["warns"] += 1 if lvl == "WARN" else 0
            lat = int(row.get("latency") or 0)
            if lat > 0: n["latencies"].append(lat)
        return name

    def add_edge(src, dst, row=None, label="calls"):
        src = _arch_clean_flow_name(src); dst = _arch_clean_flow_name(dst)
        if not src or not dst or src == dst:
            return
        key = (src, dst)
        ...
        if row:
            ...
            e["errors"] += 1 if lvl in ("ERROR", "FAILURE") else 0
            lat = int(row.get("latency") or 0)
            if lat > 0: e["latencies"].append(lat)

    add_node("Client", "Client")
    add_node("Response", "Client")

    for r in rows or []:
        app = _arch_clean_flow_name(r.get("app") or api_name or "Unknown-API") or "Unknown-API"
        flow = _arch_flow_from_row(r)
        msg = r.get("message") or ""
        trace_id = r.get("trace") or r.get("event") or ""
        ep = _arch_endpoint_from_row(r) or endpoint or "/"

        add_node(app, "API", r)
        add_edge("Client", app, r, "entry")
        if flow and flow.lower() != app.lower():
            add_node(flow, "Service", r)
            add_edge(app, flow, r, "executes")
            add_edge(flow, "Response", r, "returns")
            svc = flow
        else:
            add_edge(app, "Response", r, "returns")
            svc = app

        if trace_id:
            tr = traces.setdefault(trace_id, {"trace": trace_id, "rows": [], "errors": 0, "latency": 0, "endpoint": ep, "api": app})
            tr["rows"].append({
                "time": r.get("time", ""),
                "service": svc,
                "level": r.get("level", ""),
                "action": _arch_action_from_msg(msg),
                "processor": r.get("processor_index", ""),
                "message": msg[:280],
                "latency": int(r.get("latency") or 0)
            })
            tr["errors"] += 1 if r.get("level") in ("ERROR", "FAILURE") else 0

    # Compute trace duration from first and last log timestamp when logs do not have explicit latency.
    for tr in traces.values():
        stamps = [_parse_dt_ms(x.get("time")) for x in tr.get("rows", [])]
        stamps = [x for x in stamps if x is not None]
        explicit = [int(x.get("latency") or 0) for x in tr.get("rows", []) if int(x.get("latency") or 0) > 0]
        tr["latency"] = max(explicit) if explicit else (max(stamps) - min(stamps) if len(stamps) >= 2 else 0)
        for row in tr.get("rows", []):
            row["trace_latency_ms"] = tr["latency"]

    # Add computed trace durations back onto API→Service and Service→Response edges for useful avg latency.
    for tr in traces.values():
        if tr["latency"] > 0 and tr.get("rows"):
            app = tr.get("api") or api_name or "Unknown-API"
            svc = tr["rows"][-1].get("service") or app
            for key in [(app, svc), (svc, "Response")]:
                if key in edge_map:
                    edge_map[key]["latencies"].append(tr["latency"])
    ...
    matrix = [{"from": e["from"], "to": e["to"], "calls": e["count"], "errors": e["errors"], "avg_latency_ms": e["avg_latency_ms"], "error_rate": e["error_rate"]} for e in edges[:60]]
    trace_list = sorted(traces.values(), key=lambda t: (-t["errors"], -t["latency"], -len(t["rows"])))[:12]
    tiers = sorted(set(n["tier"] for n in nodes), key=lambda t: tier_order.get(t, 9))
    hints = []
    if len(nodes) <= 4:
        hints.append("Only one MuleSoft API/subflow is visible in this upload. Add upstream gateway/client logs or downstream connector logs to render a fuller enterprise architecture.")
    if not any(n["tier"] in ("External", "Data") for n in nodes):
        hints.append("No explicit outbound dependency or DB connector log line was found. Add 'before request to <service>' / connector target / JDBC host logs for dependency-level mapping.")
    return {"nodes": nodes, "edges": edges, "traces": trace_list, "matrix": matrix, "tiers": tiers, "hints": hints}

def extract_system_map(rows: list, raw: str, env: str, session_id: int, user_id: int):
    """Create one clean map per API + HTTP method + endpoint, not per noisy processor/message text."""
    groups = {}
    for r in rows or []:
        api = _arch_clean_flow_name(r.get("app") or "unknown") or "unknown"
        ep = _arch_endpoint_from_row(r)
        method = _arch_method_from_row(r)
        key = (api, method, ep)
        groups.setdefault(key, []).append(r)

    flow_maps = []
    for (api_name, method, endpoint), ep_rows in groups.items():
        arch = extract_architecture_graph(ep_rows, raw, env, session_id, user_id, api_name, endpoint)
        steps = ["Client", api_name]
        service_nodes = [n["name"] for n in arch.get("nodes", []) if n.get("tier") == "Service"]
        for svc in service_nodes[:8]:
            if svc not in steps:
                steps.append(svc)
        steps.append("Response")
        req_count = len(ep_rows)
        err_count = sum(1 for r in ep_rows if r.get("level") in ("ERROR", "FAILURE"))
        trace_lats = [t.get("latency", 0) for t in arch.get("traces", []) if t.get("latency", 0) > 0]
        avg_lat = round(sum(trace_lats) / len(trace_lats)) if trace_lats else 0
        sample_trace = next((r.get("trace") or r.get("event") for r in ep_rows if r.get("trace") or r.get("event")), "")
        flow_maps.append(ApiFlowMap(
            user_id=user_id, session_id=session_id, api_name=api_name, environment=env,
            endpoint=endpoint if endpoint != "/" else "", method=method,
            flow_steps_json=json.dumps(steps[:14]), architecture_json=json.dumps(arch, default=str),
            request_count=req_count, error_count=err_count, avg_latency_ms=avg_lat,
            sample_trace_id=(sample_trace or "")[:120]
        ))
    return flow_maps
